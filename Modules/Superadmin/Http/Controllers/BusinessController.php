...
class BusinessController extends BaseController
{
    ...
    public function changebusiness(Request $request){
        ...
        if($request->ajax()){
            ...
            //UPDATE `neqaty`.`model_has_roles` SET `role_id` = '58' WHERE (`role_id` = '1') and (`model_id` = '1') and (`model_type` = 'App\\User');
            $user_role = $user->roles->first();
            if($user_role)
                $user->removeRole($user_role->name);

            $user->assignRole('Admin#'.$request->business_id);
            /*$role = Role::findOrFail($role_id);
            $user->assignRole($role->name);*/
            //$set=DB::table(model_has_roles)


            $output=[
                'success'=>'true',
                'msg'=>'Good go to logout'
            ];
